        ...
        public async Task<List<OrderBook>> GetAsync(string exchangeName, string instrument, DateTime from, DateTime to, CancellationToken cancelToken)
        {
            ...
            (blobName) =>
            {
                try
                {
                    ...
                    if (match.Success)
                    {
                        var orderBookTimestamp = DateTime.ParseExact(match.Value.ToOrderbookTimestamp(), Constants.OrderbookTimestampFormat, CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal);
